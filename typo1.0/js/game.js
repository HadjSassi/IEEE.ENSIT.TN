...
function finishGame() {
    ...
    // show finishing text
    quote_text.textContent = "Thank you For playing TYPO 1.0, Hope You like it our first version 😊";

    // display restart button
    btnReStart.style.display = "block";
    quote_text.appendChild(replayButton);
...
function shuffleArray(array) {
    const distinctValuesScript = document.getElementById('distinct-values');
    let distinctValues = [];

    if (distinctValuesScript) {
        distinctValues = JSON.parse(distinctValuesScript.innerHTML);
        const arrayCopy = [...array];
        let i = 0;
        while (arrayCopy.length > 0) {
            const index = distinctValues[i] % arrayCopy.length;
            newArray.push(arrayCopy.splice(index, 1)[0]); // Remove and push to newArray
            i++;
        }
    }
    return newArray;
...
function shuffleArray(array) {
    ...
    xhr.open('GET', 'random_values.json', false);
    xhr.send()
    if (xhr.status === 200) {
        const distinctValues = JSON.parse(xhr.responseText);
